...
function module(rsc, path) -- load a LUA resource file as module
  if path == nil then -- shortcut for BRE, can omit the resource parameter
    path = rsc
    rsc = "bre"
  end
  ...
  if modules[key] then -- cached module
    ...
    if f then
      ...
      if ar[1] then
        ...
        modules[key] = nil
        print("[BRE] error loading module "..rsc.."/"..path..":"..ar[2])
      end
    else
      print("[BRE] error parsing module "..rsc.."/"..path..":"..err)
    end
  end
end
