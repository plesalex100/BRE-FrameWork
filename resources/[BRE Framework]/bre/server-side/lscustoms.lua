...
AddEventHandler("lscustom:doPayment", function(price)
	...
	if BRE.payMoney({user_id, price, true}) then
		TriggerClientEvent("lscustom:sayPayment",source,2)
	else
		BREclient.notify(source, {"Not enough money!"})
		TriggerClientEvent("lscustom:sayPayment",source,3)
	end
end)

MySQL.createCommand("BRE/vehicles_lscostums","ALTER TABLE bre_vehicles ADD IF NOT EXISTS upgrades VARCHAR(255)")
MySQL.createCommand("BRE/sendDB","UPDATE bre_vehicles SET upgrades = @upgrades WHERE id = @id AND vehicle = @vehicle")

MySQL.execute("BRE/vehicles_lscostums")

RegisterServerEvent("lscustom:sendDB")
AddEventHandler("lscustom:sendDB", function(upgrades, vehicle)
		MySQL.execute("BRE/sendDB",{upgrades = upgrades,id = BRE.getUserId({source}),vehicle = vehicle})
end)