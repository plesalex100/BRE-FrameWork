...
local function tick()
  SetTimeout(1, function() -- protect errors from breaking the loop
    ...
    for id,cb in pairs(tasks) do
      local r = exports.bre_mysql:checkTask(id)
      if r.status == 1 then
...
AddEventHandler("BRE:MySQL_task", function(task_id, data)
--  print("BRE:MySQL_task "..task_id)
  ...
  if MySQL.debug and dpaths[task_id] then
    print("MySQL end query "..dpaths[task_id].." ("..task_id..")")
    dpaths[task_id] = nil
  end
end)

local task_id = -1
AddEventHandler("BRE:MySQL_taskid", function(_task_id)
--  print("BRE:MySQL_taskid ".._task_id)
  task_id = _task_id
end)

-- host can be "host" or "host:port"
function MySQL.createConnection(name,host,user,password,db,debug)
--  print("try to create connection "..name)
...
--  TriggerEvent("BRE:MySQL:createConnection", name, config)
  exports.bre_mysql:createConnection(name, config)
end

function MySQL.createCommand(path, query)
--  print("try to create command "..path)
--  TriggerEvent("BRE:MySQL:createCommand", path, query)
  exports.bre_mysql:createCommand(path, query)
end

-- generic query
function MySQL._query(path, args, mode, cb)
  -- TriggerEvent("BRE:MySQL:query", path, args)
...
--  exports.bre_mysql:query(path, args)
--  print("try to query "..path.." id "..task_id)
  TriggerEvent("BRE:MySQL_query", path, args, mode)
  if MySQL.debug then
    print("MySQL begin query (m"..mode..") "..path.." ("..task_id..")")
    dpaths[task_id] = path
  end
